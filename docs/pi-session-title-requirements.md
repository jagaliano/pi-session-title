...
- `/session-title fix "标题"`：立即应用清洗后的指定标题，并创建 persistent manual lock，禁止自动评估替换该标题，并默认显示绿色 `● Fixed: 标题`。
- `/session-title show`：持久化显示当前标题；固定标题使用绿色 `● Fixed: 标题`，自动、建议或非固定手动标题使用黄色 `● Title: 标题`。
